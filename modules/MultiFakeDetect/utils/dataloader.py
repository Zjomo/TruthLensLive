...
# 导入.pkl，转成torch向量格式，以字典形式输出
class FakingRecipe_Dataset(Dataset):
    def __init__(self, vid_path,dataset):
        self.dataset=dataset
        if dataset=='fakesv':
            self.data_all = pd.read_json('G:/MyDataset/Graduate/FakingRecipe/fea/fakesv/metainfo.json',orient='records',dtype=False,lines=True)
            self.vid=[]
            ...
            self.ocr_pattern_fea_path=r'G:/MyDataset/Graduate/FakingRecipe/fea/fakesv/preprocess_ocr/sam'
            self.ocr_phrase_fea_path=r'G:/MyDataset/Graduate/FakingRecipe/fea/fakesv/preprocess_ocr/ocr_phrase_fea.pkl'
            with open(self.ocr_phrase_fea_path, 'rb') as f:
                self.ocr_phrase = torch.load(f)

            self.text_semantic_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakesv/preprocess_text/sem_text_fea.pkl'
            with open(self.text_semantic_fea_path, 'rb') as f:
                self.text_semantic_fea = torch.load(f)

            self.text_emo_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakesv/preprocess_text/emo_text_fea.pkl'
            with open(self.text_emo_fea_path, 'rb') as f:
                self.text_emo_fea = torch.load(f)

            self.audio_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakesv/preprocess_audio'
            self.visual_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakesv/preprocess_visual'

        elif dataset=='fakejm':
            self.data_all = pd.read_json('G:/MyDataset/Graduate/FakingRecipe/fea/fakejm/metainfo.json',orient='records',dtype=False,lines=True)
            self.vid=[]
            with open(vid_path, "r") as fr:
                for line in fr.readlines():
                    self.vid.append(line.strip())
            ...
            self.ocr_pattern_fea_path=r'G:/MyDataset/Graduate/FakingRecipe/fea/fakejm/preprocess_ocr/sam'
            self.ocr_phrase_fea_path=r'G:/MyDataset/Graduate/FakingRecipe/fea/fakejm/preprocess_ocr/ocr_phrase_fea.pkl'
            with open(self.ocr_phrase_fea_path, 'rb') as f:
                self.ocr_phrase = torch.load(f)

            self.text_semantic_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakejm/preprocess_text/sem_text_fea.pkl'
            with open(self.text_semantic_fea_path, 'rb') as f:
                self.text_semantic_fea = torch.load(f)

            self.text_emo_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakejm/preprocess_text/emo_text_fea.pkl'
            with open(self.text_emo_fea_path, 'rb') as f:
                self.text_emo_fea = torch.load(f)

            self.audio_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakejm/preprocess_audio'
            self.visual_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakejm/preprocess_visual'

        elif dataset=='fakett':
            self.data_all=pd.read_json('G:/MyDataset/Graduate/FakingRecipe/fea/fakett/metainfo.json',orient='records',lines=True,dtype={'video_id': str})
            self.vid=[]
            with open(vid_path, "r") as fr:
                for line in fr.readlines():
                    self.vid.append(line.strip())
            self.data = self.data_all[self.data_all.video_id.isin(self.vid)]
            self.data.reset_index(inplace=True)
            self.ocr_pattern_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakett/preprocess_ocr/sam'

            self.ocr_phrase_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakett/preprocess_ocr/ocr_phrase_fea.pkl'
            with open(self.ocr_phrase_fea_path, 'rb') as f:
                self.ocr_phrase = torch.load(f)

            self.text_semantic_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakett/preprocess_text/sem_text_fea.pkl'
            with open(self.text_semantic_fea_path, 'rb') as f:
                self.text_semantic_fea = torch.load(f)

            self.text_emo_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakett/preprocess_text/emo_text_fea.pkl'
            with open(self.text_emo_fea_path, 'rb') as f:
                self.text_emo_fea = torch.load(f)

            self.audio_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakett/preprocess_audio'
            self.visual_fea_path='G:/MyDataset/Graduate/FakingRecipe/fea/fakett/preprocess_visual'
    ...
    for i in range(len(lst)):
        ...
        if seg_len>=seq_len:
            ...
            if ori_len<seq_len:
                ...
                sample_index = sample_index // 50     # 视频抽帧：默认50
