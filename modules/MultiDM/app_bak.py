...
def create_interface():
    ...
    theme = gr.themes.Soft(
      ...
      shadow_drop="0 12px 30px rgba(225,29,72,0.08), 0 4px 10px rgba(17,24,39,0.06)",
      button_primary_background_fill="#dc2626",  # red-600
      button_primary_background_fill_hover="#b91c1c",
      input_border_color="#fecdd3"  # rose-200 细描边
    )

    # —— 全局 CSS：白红配色 + 卡片化布局 + 聚焦红色高亮 —— #
    css = r"""
    .main-container { max-width: 1200px; margin: 0 auto; }

    /* 卡片：白底 + 红系描边 + 柔影（竞赛观感） */
    .video-section, .chat-section, .button-section {
      border: 1px solid #fecdd3;         /* rose-200 */
      border-radius: 16px;
      background: #ffffff;
      box-shadow: 0 12px 30px rgba(225,29,72,.08), 0 4px 10px rgba(17,24,39,.06);
      padding: 20px; margin: 10px 0;
    }
    .chat-section { background: #fff1f2; }  /* rose-50，弱分区 */

    .status-box { padding: 10px; border-radius: 10px; margin: 10px 0; }
    .success-status { background:#dcfce7; border:1px solid #86efac; color:#166534; }
    .error-status   { background:#fee2e2; border:1px solid #fecaca; color:#7f1d1d; }

    /* 操作区更醒目一点 */
    .button-section { background: #fff7f8; } /* 更浅，突出“操作区” */

    /* 输入聚焦：红色环（无障碍高对比） */
    textarea:focus, input:focus, select:focus {
      box-shadow: 0 0 0 4px rgba(244,63,94,.18); /* rose-500 alpha */
      border-color: #fb7185 !important;          /* rose-400 */
    }

    /* 终端日志保留原黑底风格，仅圆角统一 */
    pre, .gr-textbox textarea { border-radius: 12px; }

    /* 轻量骨架屏与滚动条：红系微交互 */
    ::-webkit-scrollbar{height:10px;width:10px}
    ::-webkit-scrollbar-thumb{background:#fb7185;border-radius:8px}
    .skeleton{
    ...
    /* 弹窗遮罩与卡片（保留你的逻辑，仅统一圆角与投影） */
    #modal { position: fixed; inset: 0; background: rgba(0,0,0,.45); z-index: 9999; }
    #modal .modal-card {
      position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
      background: #fff; padding: 20px; border-radius: 14px;
      min-width: 320px; max-width: 520px; width: min(90vw, 520px);
      box-shadow: 0 20px 50px rgba(0,0,0,.2);
    }
    #backdrop { position:absolute; inset:0; background: transparent; border:none; cursor: default; }
    """
